...
interface IUniqueAddressesHelper {
    function uniqueAddresses(address[] memory) external view returns (address[] memory);
}
...
contract PairsHelper {
    ...
    constructor(address _wethAddress, address _uniqueAddressesHelperAddress) {
        ...
        uniqueAddressesHelper = IUniqueAddressesHelper(uniqueAddressesHelperAddress);
    ...
    function pagesLength(address factoryAddress, uint256 pageSize) public view returns (uint256) {
        uint256 _pairsLength = pairsLength(factoryAddress);
    ...
    function pairsAddresses(
        ...
        for (; pairIdx + startIdx <= endIdx; pairIdx++) {
            address pairAddress = IUniswapV2Factory(factoryAddress).allPairs(pairIdx + startIdx);
