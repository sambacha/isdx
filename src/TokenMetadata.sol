...
contract BalancesHelper {
     ...
     */
    function tokensBalances(address accountAddress, address[] memory tokensAddresses)
        public
        view
        returns (TokenBalance[] memory)
    {
        TokenBalance[] memory _tokensBalances = new TokenBalance[](tokensAddresses.length);
        for (uint256 tokenIdx = 0; tokenIdx < tokensAddresses.length; tokenIdx++) {
            address tokenAddress = tokensAddresses[tokenIdx];
            IERC20 token = IERC20(tokenAddress);
            uint256 balance = token.balanceOf(accountAddress);
            uint256 priceUsdc = IOracle(oracleAddress).getPriceUsdcRecommended(tokenAddress);
    ...
    {
        TokenPrice[] memory _tokensPrices = new TokenPrice[](tokensAddresses.length);
        for (uint256 tokenIdx = 0; tokenIdx < tokensAddresses.length; tokenIdx++) {
            address tokenAddress = tokensAddresses[tokenIdx];
    ...
    {
        TokenMetadata[] memory _tokensMetadata = new TokenMetadata[](tokensAddresses.length);
        for (uint256 tokenIdx = 0; tokenIdx < tokensAddresses.length; tokenIdx++) {
            address tokenAddress = tokensAddresses[tokenIdx];
