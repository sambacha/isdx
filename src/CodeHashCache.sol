...
contract CodeHashCache {
    ...
    function registerCodeHash(address target) external {
        ...
        assembly {
            currentCodeSize := extcodesize(target)
        }
     ...
     */
    function matchesRegisteredCodeHash(address target)
        external
        view
        returns (bool codeHashMatchesRegisteredCodeHash)
    {
     ...
     */
    function getRegisteredCodeHash(address target)
        external
        view
        returns (bytes32 registeredCodeHash)
    {
        // Get the runtime code hash that is currently registered for the target.
